...
function formatDate(array, ip){
  ...
  return array.map(item => ({...item, added: DateTime.fromJSDate(new Date(parseInt(item.added))).setLocale(ipLookup ? ipLookup.country : 'in').toLocaleString(DateTime.DATETIME_MED_WITH_WEEKDAY)}))
}
